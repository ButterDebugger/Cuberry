...
import dev.debutter.cuberry.velocity.Velocity;
...
public class BroadcastHandler implements ProtoConnectionHandler {
    ...
    public void onReady(ProtoConnection connection) {
        Velocity.getPlugin().getLogger().info("Established global chat hook with server {}", connection.getRemoteAddress());
    ...
    public void onDisconnect(ProtoConnection connection) {
        ...
        Velocity.getPlugin().getLogger().info("The global chat hook with server {} has disconnected", connection.getRemoteAddress());
    }
